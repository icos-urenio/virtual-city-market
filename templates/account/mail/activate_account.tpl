...
<html lang="{MARKET.Lang}">
	<head>
		<meta charset="utf-8">
		<title>{LANG.Confirm your registration at} "{LANG.Virtual City Market}"</title>
		<style>
	...
	<body>
		...
		<div class="margin">
			<h1>{LANG.Confirm your registration at} "{LANG.Virtual City Market}"</h1>
			
			<p>{LANG.Dear} {USER.name} {USER.surname},<p>
			
			<p>{LANG.Please click on the provided link to confirm your registration.}</p>
			
			<p>{LANG.If the below link does not work, you can paste the following url into your browser}:</p>
			<p><a href="http://{MARKET.Server}{MARKET.LWebDir}/account/activate.html?code={ACTIVATION_CODE}">http://{MARKET.Server}{MARKET.LWebDir}/account/activate.html?code={ACTIVATION_CODE}</a></p>
