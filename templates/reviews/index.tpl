	...
	<php>
		...
		if (sqlQuery($sql, $res)) {
			...
			while ($row = sqlFetchAssoc($res)) {
				...
				if (sqlQuery($sql, $res1)) {
					...
					foreach ($tags as $tag) {
						$str .= '<li><a href="{MARKET.LWebDir}/reviews/index.html?content=tag&q='.urlencode($tag).'">' . htmlspecialchars($tag) . '</a></li>';
					}
					$str .= '</ul>';
				}
		...
		if (sqlQuery($sql, $res)) {
			...
			foreach ($cities as $city) {
				$str .= '<li><a href="{MARKET.LWebDir}/reviews/index.html?content=city&q='.urlencode($city).'">' . htmlspecialchars($city) . '</a></li>';
			}
		...
		$WHERE = "directory.id=directory_ml.id AND directory.id=directory_ps.id AND directory.id=store_data.directory_id AND store_data.type='comment' AND directory_ml.lang='" . MARKET_LANG . "' AND directory_ps.publish='1' GROUP BY directory.id ORDER BY business_title";
