	...
	<php>
		...
		if (sqlQuery($sql, $res)) {
			...
			while ($row = sqlFetchAssoc($res)) {
				...
				if (sqlQuery($sql, $res1)) {
					...
					foreach ($tags as $tag) {
						$str .= '<li><a href="{MARKET.LWebDir}/marketplace/index.html?content=tag&q='.urlencode($tag).'">' . htmlspecialchars($tag) . '</a></li>';
					}
					$str .= '</ul>';
				}
		...
		if (sqlQuery($sql, $res)) {
			...
			foreach ($cities as $city) {
				$str .= '<li><a href="{MARKET.LWebDir}/marketplace/index.html?content=city&q='.urlencode($city).'">' . htmlspecialchars($city) . '</a></li>';
			}
